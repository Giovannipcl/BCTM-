...
bctm_tensor_multi = function(A,Al,X,K1,K2,K3,q,index){
 ...
 calc_lprior = function(sigma){
  ...
  cl <- makeCluster(spec=detectCores(), type="FORK", outfile="")
  posti =  parLapply(cl,seq(1:(length(index))),teste,mu = mu_ini,sigma_beta = sigma_ini,H=H,sigmanovo = c(sigmamod))
...
bctm_vcm = function(A,Al,X,K1,K2,K3,q,index){
  ...
  d = optimParallel(par = c(0,0), fn = calc_lpost32,xin = xin, method = "L-BFGS-B",
                                  control=list(fnscale=-1,maxit = 5000,trace = TRUE),hessian = TRUE, parallel=list(loginfo=TRUE,cl = cl))
...
bctm_vcm_re2 = function(A,Al,X,K1,K2,K3,q,index){
  ...
  system.time({post_x2 =  mclapply(lista, function(alpha_) {
